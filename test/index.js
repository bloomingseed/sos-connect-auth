...
async function createAccount(){
    try{
        let admin = new Accounts({'username':'nhattungnguyen.2kgl@gmail.com','password_hash':''});
        console.log(admin);
        await admin.save();
    } catch(e){
...
async function getAccountToken(){
    ...
    console.log(`${admin.username} has ${adminToken?1:0} tokens.`,adminToken);
    admin = await Accounts.findByPk("bloomingseed");
    adminToken = admin?await admin.getTokens():null;
    console.log(`${admin?admin.username:admin} has ${adminToken?1:0} tokens.`,adminToken);
}
/**
